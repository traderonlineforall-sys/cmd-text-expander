...
enum CmdId {
    ID_SAVE = 1, ID_NEW = 2, ID_DELETE = 3, ID_COPY = 4, ID_IMPORT = 5, ID_EXPORT = 6, ID_DATA = 7,
    ID_ENABLE = 10, ID_DISABLE = 11, ID_KEYWORD = 20, ID_GROUP = 21, ID_TEXT = 22, ID_SEARCH = 30, ID_LIST = 31,
...
static void LoadSnippetsFromText(const std::wstring& json){
    ...
    for(auto& obj: objs){ std::wstring id,name; ExtractJsonString(obj,{L"uuid",L"id"},id); ExtractJsonString(obj,{L"name",L"title"},name); std::wstring dummyK,dummyT; bool isCombo=ExtractJsonString(obj,{L"keyword",L"key",L"trigger",L"shortcut",L"abbreviation"},dummyK) || ExtractJsonString(obj,{L"text",L"snippet",L"replacement",L"value",L"content",L"phrase"},dummyT); if(!id.empty()&&!name.empty()&&!isCombo) groups.push_back({id,name}); }
    for(auto& obj: objs){
        Snippet s;
        ...
        for(auto& g: groups) if(!s.group.empty() && s.group==g.first) { s.group=g.second; break; }
        s.keyword=Trim(s.keyword); s.enabled=!ExtractJsonBoolFalse(obj,L"enabled");
        if(!s.keyword.empty()&&!s.text.empty()) r.push_back(s);
    }
    if(!r.empty()) g_snips=r;
}
...
static std::wstring KeyToText(KBDLLHOOKSTRUCT* k){
    BYTE ks[256]; if(!GetKeyboardState(ks)) return L"";
    ks[k->vkCode] |= 0x80;
    wchar_t buf[8]={0};
    HKL hkl = GetKeyboardLayout(0);
    int r=ToUnicodeEx(k->vkCode,k->scanCode,ks,buf,7,0,hkl);
...
static LRESULT CALLBACK HookProc(int code, WPARAM wp, LPARAM lp){
    if(code<0||!g_enabled||g_internal) return CallNextHookEx(g_hook,code,wp,lp);
    if(wp!=WM_KEYDOWN&&wp!=WM_SYSKEYDOWN) return CallNextHookEx(g_hook,code,wp,lp);
    auto*k=(KBDLLHOOKSTRUCT*)lp;
    if(k->flags&LLKHF_INJECTED) return CallNextHookEx(g_hook,code,wp,lp);
    if(OwnWindowActive()) return CallNextHookEx(g_hook,code,wp,lp);
    if((GetAsyncKeyState(VK_MENU)&0x8000)!=0) return CallNextHookEx(g_hook,code,wp,lp);
    int mode=TriggerModeValue();
    bool ctrl=(GetAsyncKeyState(VK_CONTROL)&0x8000)!=0;
    if((mode==MODE_CTRL_SPACE||mode==MODE_BOTH)&&ctrl&&k->vkCode==VK_SPACE){ int m=FindMatch(g_buffer); if(m>=0){ g_pendingTarget=GetForegroundWindow(); g_pendingDelete=(int)g_buffer.size(); g_pendingText=g_snips[m].text; g_buffer.clear(); PostMessageW(g_hwnd,WM_EXPAND_SNIPPET,0,0); return 1; }}
    ...
    if((mode==MODE_DELIMITER||mode==MODE_BOTH)&&IsTriggerKey(k->vkCode)){ int m=FindMatch(g_buffer); if(m>=0){ g_pendingTarget=GetForegroundWindow(); g_pendingDelete=(int)g_buffer.size(); g_pendingText=g_snips[m].text; g_buffer.clear(); PostMessageW(g_hwnd,WM_EXPAND_SNIPPET,0,0); return 1; } g_buffer.clear(); return CallNextHookEx(g_hook,code,wp,lp); }
    if(ctrl) return CallNextHookEx(g_hook,code,wp,lp);
...
static LRESULT CALLBACK WndProc(HWND h,UINT m,WPARAM w,LPARAM l){
    switch(m){
    case WM_CREATE:{
        g_hwnd=h; g_dataPath=ExeDir()+L"\\snippets.json"; LoadSnippets(); AddTrayIcon();
        C(L"STATIC",L"cmd Native Text Expander",WS_CHILD|WS_VISIBLE,12,8,360,30,h,0);
        C(L"STATIC",L"Keyword + Space/Enter/Tab, or Ctrl+Space depending on trigger mode.",WS_CHILD|WS_VISIBLE,12,40,620,22,h,0);
        C(L"STATIC",L"Trigger mode",WS_CHILD|WS_VISIBLE,650,16,100,22,h,0);
        g_mode=C(L"COMBOBOX",L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,650,40,260,120,h,50);
        SendMessageW(g_mode,CB_ADDSTRING,0,(LPARAM)L"Keyword + Space/Enter/Tab"); SendMessageW(g_mode,CB_ADDSTRING,0,(LPARAM)L"Ctrl + Space only"); SendMessageW(g_mode,CB_ADDSTRING,0,(LPARAM)L"Both modes"); SendMessageW(g_mode,CB_SETCURSEL,0,0);
        C(L"BUTTON",L"Enable",WS_CHILD|WS_VISIBLE,930,38,80,30,h,ID_ENABLE); C(L"BUTTON",L"Disable",WS_CHILD|WS_VISIBLE,1020,38,80,30,h,ID_DISABLE);
        C(L"STATIC",L"Keyword",WS_CHILD|WS_VISIBLE,12,85,120,22,h,0); g_keyword=C(L"EDIT",L"",WS_CHILD|WS_VISIBLE|WS_BORDER|ES_AUTOHSCROLL,120,82,260,28,h,ID_KEYWORD);
        C(L"STATIC",L"Group",WS_CHILD|WS_VISIBLE,400,85,80,22,h,0); g_group=C(L"EDIT",L"",WS_CHILD|WS_VISIBLE|WS_BORDER|ES_AUTOHSCROLL,480,82,260,28,h,ID_GROUP);
        C(L"STATIC",L"Snippet Text",WS_CHILD|WS_VISIBLE,12,122,120,22,h,0); g_text=CreateWindowExW(WS_EX_CLIENTEDGE,L"EDIT",L"",WS_CHILD|WS_VISIBLE|ES_MULTILINE|ES_AUTOVSCROLL|WS_VSCROLL,120,120,620,96,h,(HMENU)ID_TEXT,g_inst,nullptr);
        C(L"BUTTON",L"Save",WS_CHILD|WS_VISIBLE,760,120,90,30,h,ID_SAVE); C(L"BUTTON",L"New",WS_CHILD|WS_VISIBLE,860,120,90,30,h,ID_NEW); C(L"BUTTON",L"Delete",WS_CHILD|WS_VISIBLE,960,120,90,30,h,ID_DELETE);
        C(L"BUTTON",L"Copy",WS_CHILD|WS_VISIBLE,760,158,90,30,h,ID_COPY); C(L"BUTTON",L"Import",WS_CHILD|WS_VISIBLE,860,158,90,30,h,ID_IMPORT); C(L"BUTTON",L"Export",WS_CHILD|WS_VISIBLE,960,158,90,30,h,ID_EXPORT); C(L"BUTTON",L"Data Folder",WS_CHILD|WS_VISIBLE,760,196,130,30,h,ID_DATA);
        C(L"STATIC",L"Search",WS_CHILD|WS_VISIBLE,12,260,80,22,h,0); g_search=C(L"EDIT",L"",WS_CHILD|WS_VISIBLE|WS_BORDER|ES_AUTOHSCROLL,12,286,360,28,h,ID_SEARCH);
        g_count=C(L"STATIC",L"",WS_CHILD|WS_VISIBLE,720,290,300,22,h,0); g_list=C(L"LISTBOX",L"",WS_CHILD|WS_VISIBLE|WS_BORDER|WS_VSCROLL|LBS_NOTIFY,12,325,1000,300,h,ID_LIST);
        g_status=C(L"STATIC",L"Ready",WS_CHILD|WS_VISIBLE,12,690,1000,22,h,0); RefreshList(); StartHook(); return 0; }
    ...
    case WM_COMMAND:{ int id=LOWORD(w); if(id==ID_SAVE) SaveCurrent(); else if(id==ID_NEW) ClearEditor(); else if(id==ID_DELETE){ int i=SelectedSnippetIndex(); if(g_editIndex>=0)i=g_editIndex; if(i>=0&&i<(int)g_snips.size()){ g_snips.erase(g_snips.begin()+i); SaveSnippets(); RefreshList(); ClearEditor(); }} else if(id==ID_COPY) ClipboardSet(GetText(g_text)); else if(id==ID_IMPORT) ImportFile(); else if(id==ID_EXPORT) ExportFile(); else if(id==ID_DATA) ShellExecuteW(h,L"open",ExeDir().c_str(),nullptr,nullptr,SW_SHOWNORMAL); else if(id==ID_ENABLE||id==ID_TRAY_ENABLE) StartHook(); else if(id==ID_DISABLE||id==ID_TRAY_DISABLE) StopHook(); else if(id==ID_TRAY_SHOW) ShowMain(); else if(id==ID_TRAY_EXIT) DestroyWindow(h); else if(id==ID_SEARCH && HIWORD(w)==EN_CHANGE) RefreshList(); else if(id==ID_LIST && HIWORD(w)==LBN_DBLCLK) LoadSelected(); return 0; }
    case WM_DESTROY: RemoveTrayIcon(); if(g_hook)UnhookWindowsHookEx(g_hook); PostQuitMessage(0); return 0;
    }
    return DefWindowProcW(h,m,w,l);
}

int APIENTRY wWinMain(HINSTANCE h,HINSTANCE,LPWSTR,int n){ g_inst=h; WNDCLASSW wc{}; wc.lpfnWndProc=WndProc; wc.hInstance=h; wc.hCursor=LoadCursor(nullptr,IDC_ARROW); wc.hIcon=LoadIcon(nullptr,IDI_APPLICATION); wc.hbrBackground=(HBRUSH)(COLOR_WINDOW+1); wc.lpszClassName=L"CmdNativeTextExpander"; RegisterClassW(&wc); HWND wnd=CreateWindowExW(0,wc.lpszClassName,L"cmd Text Expander",WS_OVERLAPPEDWINDOW,100,80,1120,760,nullptr,nullptr,h,nullptr); ShowWindow(wnd,n); UpdateWindow(wnd); MSG msg; while(GetMessageW(&msg,nullptr,0,0)){ TranslateMessage(&msg); DispatchMessageW(&msg);} return 0; }
