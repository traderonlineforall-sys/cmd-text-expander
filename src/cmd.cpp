...
static MatchResult BestSuffixMatch(const std::wstring& rawBuffer) {
    ...
    for (size_t i = 0; i < g_snips.size(); ++i) {
        ...
        if (cleanBuffer.size() >= key.size() &&
            cleanBuffer.compare(cleanBuffer.size() - key.size(), key.size(), key) == 0) {
            ...
            if ((int)key.size() > best.deleteCount) {
                best.index = (int)i;
                best.deleteCount = rawDelete > 0 ? rawDelete : (int)g_snips[i].keyword.size();
            }
        }
    }
    return best;
...
static void SaveCurrent() {
    ...
    if (g_editIndex >= 0 && g_editIndex < (int)g_snips.size()) g_snips[g_editIndex] = s;
    else g_snips.push_back(s);
...
int APIENTRY wWinMain(HINSTANCE h, HINSTANCE, LPWSTR, int n) {
    ...
    wc.lpszClassName = L"cmdV16Reliable";
    RegisterClassW(&wc);
