...
static std::wstring NormalizeKey(std::wstring s) {
    ...
    for (wchar_t c : s) {
        ...
        if (c >= 0x06F0 && c <= 0x06F9) c = L'0' + (c - 0x06F0);
...
static MatchResult BestSuffixMatch(const std::wstring& rawBuffer) {
    ...
    if (cleanBuffer.empty()) return best;

    for (const auto& entry : g_keyCache) {
        const std::wstring& key = entry.normalized;
        if (cleanBuffer.size() < key.size()) continue;

        if (cleanBuffer.compare(cleanBuffer.size() - key.size(), key.size(), key) == 0) {
            int rawDelete = 0;
            std::wstring rebuilt;
            ...
            best.index = entry.snippetIndex;
            best.deleteCount = rawDelete > 0 ? rawDelete : (int)g_snips[entry.snippetIndex].keyword.size();
            return best;
        }
    }
...
static void StartHook() {
    ...
    Status(g_enabled ? L"cmd enabled. Recommended mode: Ctrl + Space." : L"Hook failed. Try Run as administrator.");
}
...
static LRESULT CALLBACK WndProc(HWND h, UINT m, WPARAM w, LPARAM l) {
    ...
    case WM_COMMAND: {
        ...
        if (id == ID_SAVE) SaveCurrent();
            ...
            if (i >= 0 && i < (int)g_snips.size()) { g_snips.erase(g_snips.begin() + i); RebuildKeyCache(); SaveSnippets(); RefreshList(); ClearEditor(); }
